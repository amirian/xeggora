...
public class FullAggregatedConstraint {
	final private static boolean aggregateOneLiterals = false;//TODO move this to parameters 
	private String id;
	...
	public void addConstraintAndDeleteOldOne(ILPConnector con) throws ILPException{
		if (aggregateOneLiterals || aggregatedMoreThanOneClause())
			con.addFullAggregatedConstraint(isHard, weight, aggregatedVars, aggregatingList, conjunction);
		else {//Me
			ArrayList<Literal> literals = new ArrayList<Literal>();
			literals.addAll(aggregatedVars);
			for (ArrayList<Literal> l: aggregatingList)
				literals.addAll(l);
			try {
				if (isHard)
					con.addHardConstraint(literals);
				else
					con.addSingleSoftConstraint(weight, literals, conjunction);
