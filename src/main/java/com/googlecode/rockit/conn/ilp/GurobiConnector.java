...
public class GurobiConnector extends ILPConnector{
	...
	public void addVariable(String varName, double objWeight, double lowerBound, double upperBound,
		...
		this.addVariablePrivate(varName, objWeight, lowerBound, upperBound, override, category);		
	}
	...
	private GRBVar addVariablePrivate(String varName, double objWeight, double lowerBound, double upperBound,
		...
		GRBVar var = null;
		try{
			if(category == ILPVarCategory.Z){
				this.lastZName = varName;
				if(upperBound>1){
//Me TEMP GRB.INFINITY instead of upperBound, but just when weight<0					
//					if (objWeight<0) this.lastZ = model.addVar(lowerBound, GRB.INFINITY, objWeight, GRB.INTEGER, varName); else
				...
				if(existingV != null){
					...
					if (category == ILPVarCategory.MIXED)
						numberOfMixedVariables ++;
...
*/
	...
	public void addConstraint(ArrayList<ILPVariable> lhs, ILPOperator operator, double rhs) throws ILPException {
		...
		for(ILPVariable ilpVar : lhs){
			GRBVar var = null;
			if(ilpVar.getCategory() == ILPVarCategory.Z){
				...
				var = this.addVariablePrivate(ilpVar.getName(), 0d, 0d, 1d, false, ilpVar.getCategory());
			}
...
//Me
			if(Parameters.DEBUG_OUTPUT) 
				model.write("model"+System.currentTimeMillis()+".lp");
//Me		
			if(Parameters.DEBUG_OUTPUT) 
...
Me*/	        
	...
	public void initialize() throws ILPException {
		 try{
			...
			model.set(GRB.IntParam.Threads, Parameters.THREAD_NUMBER);
			
			numberOfBinaryVariables = 0;
			
			numberOfMixedVariables = 0;
