...
public class Type implements Comparable<Type>{
	...
	public void setGroundValuesPredicate(Predicate predicate) throws ParseException{
		if(predicate==null){
			throw new ParseException("Predicates must not equal null when added to type. You tried to add a null predicate to type " + this.getName());
		}
		if(predicate.getTypes().size()>1){
			throw new ParseException("Predicates which are used to set ground values must not have more than one type. This happens when trying to assign predicate " + predicate.getName() + " to type " + this.getName());
		}
