...
public class FormulaObjective extends FormulaAbstract {
	...
	public FormulaObjective(String name, HashSet<VariableType> forVariables, ArrayList<IfExpression> ifExpressions,
			VariableDouble doubleVariable, PredicateExpression objectiveExpression) throws ParseException {
	...
	private void checkObjectiveExpression(PredicateExpression ex) throws ParseException{
		if(ex.getClass().equals(PredicateExpression.class)){
			...
			if(exPred.getPredicate().isObserved()){
				throw new ParseException("Error in the objective part of the formular. The predicate " + exPred.getPredicate().getName() + " is set to observed. But only hidden predicates are allowed in the objective part.");
			}
			if(exPred.isNegative()){
				throw new ParseException("Error in the objective part of the formular. The predicate " + exPred.getPredicate().getName() + " is set to negative. But only positive predicates are allowed in the objective part.");
			}
		}
	}
